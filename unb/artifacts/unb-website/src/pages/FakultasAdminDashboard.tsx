...
type MenuId = "konten" | "users";

const MENU_ITEMS: { id: MenuId; label: string; icon: React.ReactNode }[] = [
  { id: "konten", label: "Kelola Konten Fakultas", icon: <GraduationCap className="w-4 h-4" /> },
...
export default function FakultasAdminDashboard() {
  ...
  const [menu, setMenu] = useState<MenuId>("konten");
  ...
  const handleAddUser = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${API}/auth/users`, {
        ...
        body: JSON.stringify({ username: newUsername, password: newPassword, displayName: newDisplayName, role: "prodi", scopeId: newScopeId || null }),
      });
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-auto">
        ...
        <div className="md:hidden sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-white dark:bg-[#111] border-b border-slate-200 dark:border-slate-800 shadow-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-slate-400 truncate">{facultyName}</p>
          </div>
        </div>
        ...
        <div className="flex-1">
          {menu === "konten" && (
            <div className="p-4 md:p-8 max-w-5xl">
              <h1 className="text-xl md:text-2xl font-extrabold text-slate-800 dark:text-white mb-4 md:mb-6">{facultyName}</h1>
          ...
          {menu === "users" && (
            <div className="p-4 md:p-8 max-w-3xl space-y-6">
              <div>
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Tambah atau hapus admin yang dapat mengakses dashboard ini.</p>
              </div>

              <Card>
                <CardHeader>
                  <CardTitle className="text-base flex items-center gap-2"><Plus className="w-4 h-4 text-emerald-500" />Tambah Pengguna Baru</CardTitle>
                    ...
                    <div className="space-y-2">
                      {users.map(u => (
                        <div key={u.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl">
                          ...
                          <Button variant="ghost" size="icon" className="text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30" onClick={() => handleDeleteUser(u.id, u.username)}>
