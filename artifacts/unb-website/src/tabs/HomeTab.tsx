...
export function HomeTab({ setActiveTab, openDetail, onOpenNewsPost }: HomeTabProps) {
  ...
  // Gallery items from admin stock
  const galleryItems = useMemo<GalleryItem[]>(() => {
    const raw = (settings as any).galleryContent;
    if (raw?.items && Array.isArray(raw.items) && raw.items.length >= 3) {
      return raw.items as GalleryItem[];
    }
    return [];
  }, [settings]);

  const [gridItems, setGridItems] = useState<GalleryItem[]>(() =>
    galleryItems.length >= 3 ? shuffleAndPick(galleryItems, 3) : DEFAULT_GRID_ITEMS
  );

  // Rotate grid items every 8 seconds when gallery has stock
  useEffect(() => {
    if (galleryItems.length < 3) {
      setGridItems(DEFAULT_GRID_ITEMS);
      return;
    }
    setGridItems(shuffleAndPick(galleryItems, 3));
          ...
          {gridItems.map((item, idx) => {
            ...
            return (
              <div
                ...
                onClick={() => openDetail(item.imageUrl)}
              >
                {vid ? (
                  <video
                    src={item.imageUrl}
